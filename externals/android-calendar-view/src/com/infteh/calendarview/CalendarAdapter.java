...
public class CalendarAdapter extends BaseAdapter {
    ...
    private static Calendar closetRight;
    private static Calendar closetLeft;
    private int DAY_COUNT = 42;
    private CalendarMonthView.OnAllDaysRendered onAllDaysRendered;
    private int count;
    public boolean changedByTask;

    public static void resetCloset(){
        closetLeft = closetRight = null;
    }

    public void setUnselectableDays(ArrayList<Integer> unselectableDays, int month, int year) {
        this.unselectableDays = unselectableDays;

        Calendar date = DateHelper.createCalendar(year, month, 1, TimeZone.getDefault().getID());

        // Ищем ближайший занятый день справа
        if (closetRight == null) {
            if (mInDay != null && unselectableDays != null
                    && date.get(Calendar.MONTH) >= mInDay.get(Calendar.MONTH)
                    && date.get(Calendar.YEAR) >= mInDay.get(Calendar.YEAR)) {
                int compareDay = mInDay.getTime().before(date.getTime())
                        ? 0
                        : mInDay.get(Calendar.DAY_OF_MONTH);
                int closet = getClosetRight(compareDay, unselectableDays);
                closetRight = closet != -1
                        ? DateHelper.createCalendar(year, month, closet, TimeZone.getDefault().getID())
                        : null;
            } else {
                closetRight = null;
            }
        ...
        // Ищем ближайший занятый день слева
        if (closetLeft == null){
            if (mLeaveDay != null && unselectableDays != null
                    && date.get(Calendar.MONTH) <= mLeaveDay.get(Calendar.MONTH)
                    && date.get(Calendar.YEAR) <= mLeaveDay.get(Calendar.YEAR)) {
                int compareDay = mLeaveDay.getTime().after(date.getTime())
                        ? 32
                        : mLeaveDay.get(Calendar.DAY_OF_MONTH);
                int closet = getClosetLeft(compareDay, unselectableDays);
                closetLeft = closet != -1
                        ? DateHelper.createCalendar(year, month, closet, TimeZone.getDefault().getID())
                        : null;
            } else {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (mToday != null && dayCell.mDate.getTime().before(mToday.getTime())) {
            isSelectableDate = false;
                ...
                && closetLeft != null && dayCell.mDate.getTime().before(closetLeft.getTime())) {
            isSelectableDate = false;
        } else if (selectionType == DateSelectionType.End
                && closetRight != null && dayCell.mDate.getTime().after(closetRight.getTime())) {
            isSelectableDate = false;
...
//		}
        ...
        count++;
        if (count == DAY_COUNT && changedByTask) {
